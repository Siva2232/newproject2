...
const ProjectDetails = () => {
  ...
  if (!project) return (
    <div className="h-screen flex items-center justify-center bg-[#00162E] text-white">
      <motion.h1 
        initial={{ opacity: 0 }} 
        animate={{ opacity: 1 }} 
        className="font-serif italic text-2xl tracking-widest"
      >
  ...
  return (
    <main 
      className="
        ...
        selection:bg-[#C5A059] selection:text-black
      "
    >
      ...
      <section className="relative h-screen w-full overflow-hidden flex items-center">
        <motion.div style={{ y: imageY }} className="absolute inset-0 z-0">
          <img 
            src={project?.images?.[0] || "https://images.unsplash.com/photo-1600607687940-477a284e5773?q=80&w=2070"} 
            alt={project.title} 
            className="w-full h-full object-cover brightness-[0.7]" 
          />
          <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-[#00162E]" />
        </motion.div>
      ...
      <div className="relative z-40 -mt-24">
        <Container>
          <motion.div 
            ...
            viewport={{ once: true }} 
            className="grid grid-cols-2 md:grid-cols-4 p-8 md:p-12 gap-8 bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl"
          >
            {[
              { label: "Location", value: project.location || "California, US" },
              { label: "Year", value: project.year || "2025" },
              { label: "Typology", value: project.category || "Smart Residential" },
              { label: "Integration", value: "Full Automation" }
            ].map((stat, i) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div 
                key={i} 
                whileHover={{ y: -10 }} 
                className="p-10 border border-white/5 bg-white/[0.02] rounded-3xl group transition-all duration-500 hover:border-[#C5A059]/30"
              >
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
            <div className="lg:col-span-5 lg:sticky lg:top-32 h-fit">
              ...
              <p className="text-3xl font-serif leading-tight text-white mb-10 italic">
                "The invisible architecture of data."
              </p>
              ...
              <div className="space-y-8 border-l border-white/10 pl-8 ml-2">
                {[
                  ...
                  <div key={i} className="relative">
                    ...
                    <span className="block text-[10px] text-[#C5A059] font-bold mb-1 tracking-widest">
                      {item.step}
                    </span>
            ...
            <div className="lg:col-span-7 space-y-32">
              {["https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?q=80&w=1974", "https://images.unsplash.com/photo-1558002038-1055907df827?q=80&w=2070"].map((img, index) => (
                <motion.div 
                  ...
                  viewport={{ once: true }} 
                  className="group relative overflow-hidden rounded-2xl aspect-[4/5] md:aspect-video"
                >
                  <img 
                    src={img} 
      ...
      <footer className="py-60 relative overflow-hidden border-t border-white/5">
        <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none ml-[10px]">
          <h2 className="text-[20vw] font-black text-white whitespace-nowrap leading-none select-none -translate-x-1/4">
            NEXT PROJECT
          </h2>
        </div>
        <Container className="relative z-10 text-center">
          <Link to={`/projects/${parseInt(id) + 1}`} className="group inline-block">
            <span className="text-[10px] uppercase tracking-[0.6em] text-[#C5A059] mb-8 block">
              Continue Discovery
            </span>
            <div className="flex items-center justify-center gap-6">
              <h3 className="text-6xl md:text-9xl font-serif italic text-white/30 group-hover:text-white transition-all duration-700">
                Next Case
              </h3>
              <ArrowUpRight 
                size={80} 
                strokeWidth={1} 
                className="text-[#C5A059] group-hover:rotate-45 transition-transform duration-700 hidden md:block" 
              />
            </div>
          </Link>
        </Container>
