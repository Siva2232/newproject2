...
const SmartLivingDetails = () => {
  ...
  return (
    <>
      {/* REFINED HERO */}
      <section className="relative pt-40 pb-20 bg-[#001021] text-white overflow-hidden">
        <Container>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mb-20">
            ...
            <h1 className="text-6xl md:text-8xl font-serif leading-[0.9]">{item.title}</h1>
          </motion.div>

          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            className="relative w-full h-[60vh] rounded-[2rem] overflow-hidden"
          >
      ...
      <section className="py-24 bg-[#F5F5F3] text-stone-900">
        <Container>
          <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-8">
              <p className="text-2xl font-serif text-stone-800 leading-relaxed mb-20">
                {item.details}
              </p>
              
              {item.sections?.map((sec, si) => (
                <div key={si} className="mb-20 border-l-2 border-[#C5A059] pl-8">
                  <h2 className="text-3xl font-serif mb-6 text-stone-900">{sec.title}</h2>
      ...
      <section className="py-32 bg-[#F9F9F7]">
        <Container>
          <div className="flex items-end justify-between mb-16">
            <h3 className="text-4xl font-serif">Related Solutions</h3>
            <Link to="/smart-living" className="text-[#C5A059] flex items-center gap-2 text-sm font-bold uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {SMART_LIVING_SOLUTIONS.filter((_, i) => i !== idx).slice(0, 3).map((s, i) => (
              <Link key={i} to={`/smart-living/${SMART_LIVING_SOLUTIONS.indexOf(s)}`} className="group relative block overflow-hidden rounded-2xl h-[350px]">
